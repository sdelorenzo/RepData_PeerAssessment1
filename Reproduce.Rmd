...
The daily mean is `r dmean` steps and the daily median is `r dmedian` steps.
...
#determine which interval contains on average the max number of steps
      ...
      maxrow <- maxrow[[1,1]]
```
Interval `r maxrow` contains on average the most steps.
...
```
There are `r missingcount` NA values in the original data set.
...
```
The imputed daily mean is `r idmean` (vs. original `r dmean`) steps and the daily median is `r idmedian` (vs. original `r dmedian`) steps.
Imputing the data increases the daily average by `r dailydiff` steps.
